...
if multi_layer == 1
    error('Try to run a sub-function ''beam_shape_coeff.m'', which is design for single layer media only!\n');
end
...
if strcmp(wave_type, 'plain') == 1 && ((theta_x == 0) && (theta_y == 0) && (theta_z == 0))
    % the middle three conditions are limited the axisymmetric aixs of the
    % irregular body consistent with the z-axis (without angle deviation
...
% (BN, BETA, X0, Y0, Z0)
if strcmp(wave_type, 'zero-Bessel') == 1 && ((theta_x == 0) && (theta_y == 0) && (theta_z == 0))
    % the middle three conditions are limited the axisymmetric aixs of the
    % irregular body consistent with the z-axis (without angle deviation
    % from normal position)  
    if strcmp(direction, 'Z') == 1
        Anm = p_0*4*pi * (1i)^(n) * sqrt(((2*n+1)/(4*pi)) * (factorial(n-m)/factorial(n+m))) * sLegendre(n,m,BETA) * (m==0);
        anm = Anm / ((2*n + 1) * 1i^n);  
        return;
    end
end
if strcmp(wave_type, 'non-zero-Bessel') == 1 && ((theta_x == 0) && (theta_y == 0) && (theta_z == 0))
    % the middle three conditions are limited the axisymmetric aixs of the
    % irregular body consistent with the z-axis (without angle deviation
    % from normal position)  
    if strcmp(direction, 'Z') == 1
        Anm = p_0*4*pi * (1i)^(n-BN) * sqrt(((2*n+1)/(4*pi)) * (factorial(n-m)/factorial(n+m))) * sLegendre(n,m,BETA) * (m==BN);
        anm = Anm / ((2*n + 1) * 1i^n); 
        return;
    end
end
...
% figure_FontSize = 25;
% 
% [THETA_anti, PHI] = meshgrid(theta(end:-1:1), phi);
% % Amplitude distribution on the sperical integration surface
% pclr1 = pcolor(PHI, THETA_anti, (imag(p_b)));
% set(pclr1, 'edgecolor', 'none');
% title(['\rm{\fontname{Times new roman}{For radius axis is }}', num2str(roundn(b*1000,-2)), ' \rm{\fontname{Times new roman}{[mm]}}']);
% xlabel('\rm{\fontname{Times new roman}{Azimuth angle axis} [бу]}');
% ylabel('\rm{\fontname{Times new roman}{Polar angle axis} [бу]}');
% set(get(gca,'title'),'FontSize',figure_FontSize);
