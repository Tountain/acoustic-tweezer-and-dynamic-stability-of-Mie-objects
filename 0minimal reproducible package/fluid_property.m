...
if strcmp(fluid, 'water') == 1
    ...
    fluid_viscosity = 0.89 * 10^-3;      % dynamic viscosity Pa*s
end
if strcmp(fluid, 'co2') == 1
    fluid_c = 268;              % sound speed; m/s
    atten_coe = 0.12*omega/(fluid_c*2*pi);
    atten_coe = 10;
    fluid_k = omega / fluid_c + atten_coe * 1i;  % wave number
    fluid_rho = 1.842;          % density; kg/m3
    fluid_viscosity = 0+eps;        % Pa*s
end
