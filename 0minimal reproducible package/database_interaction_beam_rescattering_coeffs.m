...
if multi_particle ~= 1
    error('Database ''database_interaction_beam_rescattering_coeffs.m'' is trying to established for single particle case!');
end
...
% scattering coefficient for multi-particle system: db_b_s_coeff
if irregular_body ~= 1
...
elseif irregular_body == 1
%     db_ibs_coeff = 'Not used in cases of multiple irregular particles system.'; 
     
    % rearrange the "A" based on the database projecting rule to "db_bs_coeff"
    db_bs_coeff = zeros(N+1, 2*N+1, particle_number);
    index = 0;
    for ii = 1 : particle_number
        for nn = 0 : N
            for mm = -nn : nn
                index = index + 1;
                db_bs_coeff(nn+1, nn+mm+1, ii) = A(index);
            end
        end
    end
     
%     db_b_s_coeff = zeros(size(db_bs_coeff)); 
%     for ii = 1:particle_number
%         for nn = 0:N
%             for mm = -nn:nn
%                 db_b_s_coeff(nn+1, nn+mm+1, ii) = db_bs_coeff(nn+1, nn+mm+1, ii) * db_s_coeff(nn+1, nn+mm+1, ii);
%             end
%         end
%     end
    
    db_b_s_coeff = db_s_coeff; 
end
...
if irregular_body == 1
    db_ibs_coeff = db_bs_coeff + db_rs_in_coeff(:,:,1);
end
