...
% NOTE: for single layer media ONLY!
%%
...
%                 % limited the axisymmetric aixs of the irregular body
%                 % consistent with the z-axis (without angle deviation from 
%                 % normal position
...
%             else
            ...
            z_rot_t = @(r, theta, phi) z_rot(r, theta, phi) + z_translation;

            p_i = @(r,theta,phi) p_0*exp(1i*fluid_k*(z_rot_t(r, theta, phi)));
%             end
        elseif rot_matrix == 1
            % this condition means we will later employ the addition rotation theorem
            % to modulate the beam-shape coefficients from OCS to CCS.
            p_i = @(r,theta,phi) p_0*ones(length(phi),1)*exp(dir_sign * 1i*fluid_k*(r*cos(theta) + z_translation));
        end
    end
end
...
if strcmp(wave_type, 'phase_array_transducer') == 1 || strcmp(wave_type, 'phase_array_transducer_standing') == 1
    ...
    %         p_i = @(r, theta, phi) p_0;
        elseif rot_matrix == 1
            ...
            z = @(r, theta, phi) r*ones(length(phi),1)*cos(theta);
            % making NO rotation
            x_rot = @(r, theta, phi) x(r, theta, phi) * 1 + y(r, theta, phi) * 0 + z(r, theta, phi) * 0;
            y_rot = @(r, theta, phi) x(r, theta, phi) * 0 + y(r, theta, phi) * 1 + z(r, theta, phi) * 0;
            z_rot = @(r, theta, phi) x(r, theta, phi) * 0 + y(r, theta, phi) * 0 + z(r, theta, phi) * 1;
            % making translations
            x_rot_t = @(r, theta, phi) x_rot(r, theta, phi) + x_translation;
            y_rot_t = @(r, theta, phi) y_rot(r, theta, phi) + y_translation;
            z_rot_t = @(r, theta, phi) z_rot(r, theta, phi) + inter_dist + z_translation;
