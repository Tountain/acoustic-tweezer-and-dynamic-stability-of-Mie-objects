...
%
% my deriviation (a)&(b)&Exactly solution [testing_type == 1&2&3] are
% coincide with G.T.S@2011(JASA) [testing_type == 4]
%
% NOTE: This function ONLY works for conditions of wave positive direction
% along Z-axis. (Therefore, ONLY works for single particle system)
%%
...
if multi_particle == 0      % for single spherical and axisymmetric object
    ...
    load([db_filename, '.mat'], 'N', 'db_bs_coeff', 'db_s_coeff', 'transducer', 'A_delay', 'phi_delay');
else
    if irregular_body ~= 1      % for multi spherical objects
        [db_filename] = database_beam_scattering_coeffs();      
        load([db_filename, '.mat'], 'N', 'db_bs_coeff', 'db_s_coeff', 'transducer', 'A_delay', 'phi_delay');
    ...
    elseif irregular_body == 1                        % for multi axisymmetric objects
        [db_filename] = database_beam_scattering_coeffs();      
        load([db_filename, '.mat'], 'N', 'db_bs_coeff', 'db_s_coeff', 'transducer', 'A_delay', 'phi_delay');
        [db_filename_ts] = database_translation_coeffs(N);
        load([db_filename_ts, '.mat'], 'Snmvu_1', 'Snmvu_2', 'r_ij');
        [db_filename_ibs] = database_interaction_beam_rescattering_coeffs(N, db_s_coeff, Snmvu_1, Snmvu_2, r_ij); 
        load([db_filename_ibs, '.mat'], 'db_rs_in_coeff', 'db_ibs_coeff', 'db_b_s_coeff');
        
%         db_bs_coeff = db_bs_coeff + db_rs_in_coeff(:,:,1);      % "1" means probe particle
%         db_s_coeff_1 = db_s_coeff(:,:,1);                       % "1" means probe particle
%         db_s_coeff = db_s_coeff_1;
        
%         db_S_coeff = db_s_coeff(:,:,1) .* db_bs_coeff;         % "1" means probe particle
%         db_bs_coeff = db_bs_coeff + db_rs_in_coeff(:,:,1);      % "1" means probe particle
%         db_s_coeff = zeros(N+1, 2*N+1);
%         for nn = 0 : N
%             for mm = -nn : nn
%                 if db_bs_coeff(nn+1, nn+mm+1) == 0
%                     db_s_coeff(nn+1, nn+mm+1) = 0;
%                 else
%                     db_s_coeff(nn+1, nn+mm+1) = db_S_coeff(nn+1, nn+mm+1)/db_bs_coeff(nn+1, nn+mm+1);
%                 end
%             end
%         end

        db_bs_coeff = db_ibs_coeff(:,:,1);      % "1" means probe particle
        db_s_coeff = db_b_s_coeff(:,:,1);
        for nn = 0 : N
            for mm = -nn : nn
                if db_bs_coeff(nn+1, nn+mm+1) == 0
                    db_s_coeff(nn+1, nn+mm+1) = 0;
                else
                    db_s_coeff(nn+1, nn+mm+1) = db_b_s_coeff(nn+1, nn+mm+1)/db_bs_coeff(nn+1, nn+mm+1);
                end
            end
        end
...
if multi_layer == 1
    fluid_c = c_layer; 
    fluid_rho = density_layer;
    fluid_k = 2*pi*freq / c_layer;
end
...
%                 middle_var = db_bs_coeff(nn+1,mm+nn+1) + db_s_coeff(nn+1, nn+mm+1);
        ...
        if abs(m_plus_1) <= n_plus_1    % make sure the adjusting position within the database
            ...
            elseif irregular_body == 1
                ...
                sum_1 = sum_1 + db_bs_coeff(nn+1,mm+nn+1) * middle_var * part_AA;
%                 part_AA = 1 * conj(db_s_coeff(n_plus_1+1,m_plus_1+n_plus_1+1)) * AA(nn+1,mm+1);
%                 sum_1 = sum_1 + 1 * middle_var * part_AA;
            end
        end
...
%                 middle_var = db_bs_coeff(nn+1,mm+nn+1) + db_s_coeff(nn+1, nn+mm+1);
        ...
        if abs(m_plus_1) <= n_minus_1
            ...
            elseif irregular_body == 1
                ...
                sum_2 = sum_2 + db_bs_coeff(nn+1,mm+nn+1) * middle_var * part_BB;
%                 part_BB = 1 * conj(db_s_coeff(n_minus_1+1,m_plus_1+n_minus_1+1)) * BB(nn-1,mm+1);
%                 sum_2 = sum_2 + 1 * middle_var * part_BB;
            end
        end
...
%                 middle_var = db_bs_coeff(nn+1,mm+nn+1) + db_s_coeff(nn+1, nn+mm+1);
        ...
        if abs(m_minus_1) <= n_plus_1
            ...
            elseif irregular_body == 1
                ...
                sum_3 = sum_3 + db_bs_coeff(nn+1,mm+nn+1) * middle_var * part_CC;
%                 part_CC = 1 * conj(db_s_coeff(n_plus_1+1,m_minus_1+n_plus_1+1)) * CC(nn+1,mm-1);
%                 sum_3 = sum_3 + 1 * middle_var * part_CC;
            end
        end
...
%                 middle_var = db_bs_coeff(nn+1,mm+nn+1) + db_s_coeff(nn+1, nn+mm+1);
        ...
        if abs(m_minus_1) <= n_minus_1
            ...
            elseif irregular_body == 1
                ...
                sum_4 = sum_4 + db_bs_coeff(nn+1,mm+nn+1) * middle_var * part_DD;
%                 part_DD = 1 * conj(db_s_coeff(n_minus_1+1,n_minus_1+m_minus_1+1)) * DD(nn-1,mm-1);
%                 sum_4 = sum_4 + 1 * middle_var * part_DD;
            end
        end
...
%                 middle_var = db_bs_coeff(nn+1,mm+nn+1) + db_s_coeff(nn+1, nn+mm+1);
        ...
        if abs(mm) <= n_plus_1
            ...
            elseif irregular_body == 1
                ...
                sum_1 = sum_1 + db_bs_coeff(nn+1,mm+nn+1) * middle_var * part_EE;
%                 part_EE = 1 * conj(db_s_coeff(n_plus_1+1,mm+n_plus_1+1)) * EE(nn+1,mm);
%                 sum_1 = sum_1 + 1 * middle_var * part_EE;
            end
        end
...
%                 middle_var = db_bs_coeff(nn+1,mm+nn+1) + db_s_coeff(nn+1, nn+mm+1);
        ...
        if abs(mm) <= n_minus_1
            ...
            elseif irregular_body == 1
                ...
                sum_2 = sum_2 + db_bs_coeff(nn+1,mm+nn+1) * middle_var * part_FF;
%                 part_FF = 1 * conj(db_s_coeff(n_minus_1+1,mm+n_minus_1+1)) * FF(nn-1,mm);
%                 sum_2 = sum_2 + 1 * middle_var * part_FF;
            end
        end
...
if (strcmp(wave_type, 'single_transducer') || strcmp(wave_type, 'phase_array_transducer') || strcmp(wave_type, 'phase_array_transducer2')) ~= 1
    ...
    if multi_particle == 0
        if multi_layer == 0
            save([dir_file_forces_trans, 'A.mat'], 'E0', 'pi_a2_E0', 'transducer', 'A_delay', 'phi_delay', 'Frad_x', 'Frad_y', 'Frad_z');
        elseif multi_layer == 1
            save([dir_file_forces_trans, 'A.mat'], 'E0', 'pi_a2_E0', 'transducer', 'A_delay', 'phi_delay', 'Frad_x', 'Frad_y', 'Frad_z', 'c_layer', 'density_layer' , 'depth_fun');
        end
    else
        particles_Cartesian_data;
        ...
        if multi_layer == 0
            save([dir_file_forces_trans, 'A.mat'], 'E0', 'pi_a2_E0', 'transducer', 'A_delay', 'phi_delay', ...
                'Frad_x', 'Frad_y', 'Frad_z', 'particle', 'multi_particle_radius');
        elseif multi_layer == 1
            save([dir_file_forces_trans, 'A.mat'], 'E0', 'pi_a2_E0', 'transducer', 'A_delay', 'phi_delay', ...
                'Frad_x', 'Frad_y', 'Frad_z', 'particle', 'multi_particle_radius', 'c_layer', 'density_layer' , 'depth_fun');
